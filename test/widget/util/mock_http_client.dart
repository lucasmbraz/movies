...
MockHttpClient createMockImageHttpClient(io.SecurityContext _) {
  ...
  when(response.listen(typed(any), onError: anyNamed('onError'), onDone: anyNamed("onDone"), cancelOnError: anyNamed("cancelOnError"))).thenAnswer((Invocation invocation) {
...
T anyNamed<T>(String name) {
  typed(any, named: name);
  return null;
}
